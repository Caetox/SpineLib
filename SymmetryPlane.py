...
def fit_symmetry_plane(geometry: vtk.vtkPolyData, numIterations: int):
    ...
    # initial plane
    p0 = calcInitalPlane(points, centerOfMass)
    ...
    for i in range(numIterations):
        ...
        mirroredGeometryPoints = vtk_to_numpy(mirroredGeometry.GetPoints().GetData())
        #evaluateRegistrationICP(mirroredGeometryPoints, points)
...
def calcInitalPlane(points, centerOfMass):
    ...
    sorted_eigenvects = conv.closest_vector(np.concatenate((eigenvects, -eigenvects), axis=0), [1,0,0])
